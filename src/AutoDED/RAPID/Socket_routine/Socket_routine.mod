...
!// sampling rate
PERS num maintaskWaitTime:= 0.02;  !Recommended for real controller
...
! PROC Check_Command_Changed (num command)
!   VAR num temp;
!   temp := command - last_routine_command;
  
!   IF temp <> 0 THEN
!     Command_Changed:= TRUE;
!     last_routine_command:=command;
!   ENDIF
! ENDPROC
 ...
 !//Local variables
  ...
  !//Server Loop
    WHILE TRUE DO
        ...
        ! Check_Command_Changed routine_command;
