...
#include <simtech_kuka_eki_interfaceB/simtech_kuka_eki_interfaceB.h>
...
void KukaEkiHardwareInterface::init()
{
  ...
  const std::string param_addr = "eki_B/robot_address";
  const std::string param_port = "eki_B/robot_port";
  const std::string param_socket_timeout = "eki_B/socket_timeout";
  // const std::string param_max_cmd_buf_len = "eki_B/max_cmd_buf_len";

  if (nh_.getParam(param_addr, eki_server_address_) &&
      nh_.getParam(param_port, eki_server_port_))
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Configuring Kuka EKI hardware interface on: "
  ...
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Configuring Kuka EKI hardware interface B socket timeout to "
                          << eki_read_state_timeout_ << " seconds");
  }
  else
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Failed to get EKI socket B timeout from parameter server (looking "
  ...
  // {
  //   ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Configuring Kuka EKI hardware interface maximum command buffer "
  //                         "length to " << eki_max_cmd_buff_len_);
  // }
  // else
  // {
  //   ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Failed to get EKI hardware interface maximum command buffer length "
  ...
  ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Loaded Kuka EKI hardware interface B");
}


void KukaEkiHardwareInterface::start()
{
  ROS_INFO_NAMED("simtech_kuka_eki_interfaceB", "Starting Kuka EKI hardware interface B...");

  // Start client
  ROS_INFO_NAMED("simtech_kuka_eki_interfaceB", "... connecting to robot's EKI server B...");
  ...
  ROS_INFO_NAMED("simtech_kuka_eki_interfaceB", "... done. EKI hardware interface B started!");
}
...
void KukaEkiHardwareInterface::read(const ros::Time &time, const ros::Duration &period)
{
  ...
  // ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interfaceB", "Received message from hardware interface");
}
