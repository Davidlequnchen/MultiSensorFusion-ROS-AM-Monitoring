...
class NdPCLHandler():
    def __init__(self):
        ...
        #self.interval = 5.0 
        self.interval = rospy.Duration(2) # five seconds duration
    ...
    def sub_cloud_topic(self, data):
        ...
        """
        #stamp = time.time()
        stamp = data.header.stamp # current time of the message
        ...
        if (stamp - self.stamp < self.interval):
            ...
            command_line =  os.path.join(path, 'PCL_segmentation/build', "PCL_segmentation") + " -multiPlannarSeg \
