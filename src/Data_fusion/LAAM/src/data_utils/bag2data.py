'''
Author: Chen Lequn
Email: chen1470@e.ntu.edu.sg
Reference: from OpenLMD, cyplam
'''
...
def extract(filename):
    # input: a .bag file
    ## extract all the info from this file into a directory
    path = os.getcwd() #returns current working directory
    name = os.path.splitext(filename)[0] #split ddd/ccc/xxx.txt --> ddd/ccc/xxx. 
    basename = os.path.basename(name) #xxx
    directory = os.path.join(path, basename) # ddd/ccc/xxx
    if os.path.exists(directory):
        shutil.rmtree(directory) ## Delete an entire directory tree.
    os.mkdir(directory) # create a directory with the same name as the filename

    bag = rosbag.Bag(filename)
    topicList = readBagTopicList(bag) # extract all the topics from the bag
    while True:
        if len(topicList) == 0:
...
def readBagTopicList(bag):
    '''
    retrieve a list of topic with message from the bag
    '''
